...
class Cleaning():
    ...
    def main(self, code):
        ...
        if self.code == 2:
            ...
            open = int(input("개봉했는가(예는 1, 아니오는 2)?: "))
            if open == 1:
                self.pro_name = "우리 곡물 파우더(개봉)"
                today = datetime.datetime.now()
                self.dec_open = "개봉"
                self.open_date = today.strftime("%Y-%m-%dT%H:%M+09:00")
                self.hold_time = self.exp
            if open == 2:
                self.pro_name = "우리 곡물 파우더"
                self.dec_open = "개봉 안 함"
                self.hold_time = self.exp
        ...
        if self.code == 3:
            ...
            open = int(input("개봉했는가(예는 1, 아니오는 2)?: "))
            if open == 1:
                self.pro_name = "건조 레드 커런트(개봉)"
                today = datetime.datetime.now()
                self.dec_open = "개봉"
                self.open_date = today.strftime("%Y-%m-%dT%H:%M+09:00")
                self.hold_time = self.exp
            if open == 2:
                self.pro_name = "건조 레드 커런트"
                self.dec_open = "개봉 안 함"
                self.hold_time = self.exp
        ...
        if self.code == 8:
            ...
            if defro == 1:
                ...
                self.hold_time = (today + relativedelta(days=5)).strftime("%Y-%m-%dT%H:%M+09:00")
            if defro == 2:
                self.pro_name = "레드벨벳 뚱카롱"
                self.dec_open = "개봉 안 함"
                self.method = "냉동"
                self.hold_time = self.exp
            ...
            if self.exp == None:
                if self.hold_time == None:
                    ...
                    "properties": {
                        ...
                        "개봉 여부": {"select": {"name": self.dec_open}}
                        }
                    }
                else:
                    newPageData = {
    ...
    def final_result(self):
        if self.res.status_code == 200:
            print("기록 성공")
            print("결과:", self.pro_name, self.method, self.dec_open, self.exp, self.open_date, self.defro_start, self.defro_end,
                  self.hold_time)
        else:
            print("기록 실패...")
            print("결과:", self.pro_name, self.method, self.dec_open, self.exp, self.open_date, self.defro_start, self.defro_end,
